    ...
    private class AwaitContinuation<T>(
        ...
        override fun getCancellationState(parent: Job): CompletedExceptionally {
            ...
            if (state is Finishing) {
                val rootCause = state.rootCause
                if (rootCause != null) return CompletedExceptionally(rootCause)
            }
            if (state is CompletedExceptionally) return CompletedExceptionally(state.cause)
...
internal class JobImpl(parent: Job? = null) : JobSupport(true) {    init { initParentJobInternal(parent) }
